...
_FOCUS = {
    ...
    "基本面分析师": (
        ...
        "【重点】若快照中含 SEC EDGAR 数据（营收TTM/净利TTM/R&D/OCF），"
        "必须深度解读：营收增速、利润率、研发强度、现金流质量。"
        "【A 股增强】若有龙虎榜/解禁/大宗交易信息，必须独立成段分析："
        "(1) 龙虎榜：识别游资 vs 机构席位主导，判断短线动能延续性；"
        "(2) 解禁日历：解禁规模占流通比 > 5% 时强调抛压风险；"
        "(3) 大宗交易：折价 = 大股东减持信号，溢价 = 接盘方看好信号。"
    ...
    "情绪分析师": (
        ...
        "\n\n[A 股专属信号读法]"
        "\n· 看到「龙虎榜」字样：核心是席位结构 ——『机构席位』『沪/深股通专用』属中长线资金，"
        "『XX证券XX路』通常是游资短炒；净买入>1亿且机构主导=趋势信号，纯游资=博弈情绪。"
        "\n· 看到「大宗交易」：折价>5% 多为减持/抛压；溢价成交常是接盘/利好预期；"
        "营业部如出现机构席位需重点提示。"
        "\n· 看到「解禁」：占流通股>5% 视为重大抛压预警；解禁日前 5-10 个交易日"
        "情绪通常先行承压。"
        "\n[美股期权信号]（如有）"
        "\n· PCR(vol)>1：看跌情绪占优；<0.7：看多；ATM IV 突升=事件预期升温。"
    ),
}
...
_AGGRESSIVE_RISK_SYS = (
    ...
    "\n[资金流信号评分] 快照中可能包含「📊 资金流信号评分」表格，"
    "看到 总分>+0.3 → 可适度提高仓位；看到 大宗溢价+机构买入 → 强烈买入信号。"
    "\n[数值化决策] 必须明确引用信号评分的总分和各维度评分，给出具体的仓位调整建议。"
)
...
_CONSERVATIVE_RISK_SYS = (
    ...
    "输出完整的 Markdown 风险分析报告，300 字以上，含表格。"
    "\n[资金流信号评分] 快照中可能包含「📊 资金流信号评分」表格，"
    "看到 总分<-0.3 → 应缩仓或 reject；看到 解禁>5% + 大宗折价>5% → 强烈卖出信号。"
    "\n[数值化决策] 必须明确引用信号评分的总分和各维度评分，给出具体的风险警告和调整建议。"
)
...
_NEUTRAL_RISK_SYS = (
    ...
    "输出完整的 Markdown 风险分析报告，300 字以上，含表格。"
    "\n[资金流信号评分] 快照中可能包含「📊 资金流信号评分」表格，"
    "\n[数值化决策] 必须显式列出各维度评分，计算对冲后的净信号，给出平衡的决策建议。"
)
...
_FUND_MANAGER_SYS = (
    ...
    "\n\n[资金流信号评分] 快照中可能包含「📊 资金流信号评分」表格，"
    "\n  必须明确引用 总分 和 各维度评分，作为最终决策的核心依据之一。"
    "\n\n[资金流红线]"
    "\n· A 股: 信号评分总分<-0.5 → 必须 reject 或大幅缩仓；"
    "\n  信号评分显示 解禁>10% + 大宗折价>5% → 强烈卖出信号；"
    "\n  信号评分显示 北向连续流出 + 机构净卖出 → 提示风险溢价。"
    "\n· 美股: SEC 显示 OCF 连续两年为负 + 高 R&D 占营收>30% → 现金流压力红旗；"
    "\n  期权 PCR(vol)>1.5 + ATM IV 异动 → 注意短期事件风险。"
    "\n上述任一条命中，必须在最终裁定书的「关键考量」段落显式说明。"
)
