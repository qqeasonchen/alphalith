...
| ⭐ **自选股** | 多列表管理，实时行情刷新，一键跳转分析 |
| 💼 **持仓追踪** | 持仓盈亏实时计算，加权均价，分批买入自动合并 |
| 📰 **舆情分析** | 6 内置 + 自定义数据源（RSS/JSON/HTML），LLM 情绪打分，覆盖多市场 |
| 🔔 **预警通知** | 价格/涨跌幅条件触发，浏览器桌面通知 |
| 📦 **批量分析** | 空格分隔多标的，串行执行 + 实时进度 |
| 📋 **决策历史** | 按标的筛选 + 未读计数 badge，默认加载全部最近记录 |
| ⚙️ **系统配置** | 15 家供应商列表、数据源管理、自定义舆情源配置、账户设置 |
...
│   ├── watchlist.py     # 多列表自选股管理
│   ├── portfolio.py     # 持仓追踪 + 盈亏计算
│   ├── sentiment.py     # 多源舆情分析（6 内置 + 自定义源）
│   ├── alerts.py        # 价格/涨跌幅条件预警
│   ├── gui/
...
- ✅ **v0.2** — 港股 / 美股规则 · Web GUI · 多模型适配 · 策略回测
- ✅ **v0.3** — 自选股管理 · 持仓追踪 · 舆情情绪分析 · 预警通知 · 自定义数据源
- 📋 **v0.4** — 跨市场对比 · TradingView Webhook · 飞书/Telegram Bot · 策略市场
- 📋 **v1.0** — 实盘对接 · 社区策略共享 · 多语文档站点
