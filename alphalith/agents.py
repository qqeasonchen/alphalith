...
_AGGRESSIVE_RISK_SYS = (
    ...
    "\n[资金流加分项] 看到「机构席位」净买入 + 北向持续流入 + 大宗溢价 → 可适度提高仓位。"
)
...
_CONSERVATIVE_RISK_SYS = (
    ...
    "输出完整的 Markdown 风险分析报告，300 字以上，含表格。"
    "\n[资金流减分项] 解禁占流通>5% / 大宗折价>5% / 北向连续流出 / 龙虎榜纯游资主导"
    "\n  → 任一命中即应缩仓或要求更紧止损；多个命中考虑 reject。"
)
...
_NEUTRAL_RISK_SYS = (
    ...
    "输出完整的 Markdown 风险分析报告，300 字以上，含表格。"
    "\n[资金流权衡] 机构入 vs 解禁出 / 北向流入 vs 大股东减持 —— 必须显式列出对冲后的净信号。"
)
...
_FUND_MANAGER_SYS = (
    ...
    "\n\n[资金流红线]"
    "\n· A 股: 解禁占流通>10% 且交易员仓位>30% → 必须缩仓或 reject；"
    "\n  大宗折价>5% + 龙虎榜机构净卖出 → 视同强烈卖出信号；"
    "\n  北向连续 5 日净流出 + 个股 5 日北向减持 → 提示风险溢价。"
